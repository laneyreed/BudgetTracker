...
**Location:** `project/models/user.py`
...
**Location:** `project/models/account.py`
...
**Location:** `project/models/category.py`
...
**Location:** `project/models/transaction.py`
...
Here's how the models work together in a typical scenario:

1. **User Creation:**
   ```python
   user = User(id=1, name="Jane Smith", email="jane@example.com", currency="USD")
   ```
...
3. **Category Setup:**
   ```python
   grocery_category = Category(id=1, name="Groceries")
   ```
...
- Consider adding a Budget model to track spending limits per category
- May need to add Transaction → Account relationship validation to ensure the account belongs to the transaction's user
- Could add support for transfers between accounts (special transaction type)
- May want to support custom user-defined categories beyond the defaults